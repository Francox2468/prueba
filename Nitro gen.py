...
class NitroGen: # Initialise the class
    ...
    def main(self): # The main function contains the most important code
        ...
        if os.name == "nt": # If the system is windows
            print("")
            ctypes.windll.kernel32.SetConsoleTitleW("Nitro Generator and Checker - Made by: KerfiveXᴺᴬ・☠#7273") # Change the
        else: # Or if it is unix
            print(f'\33]0;Nitro Generator and Checker - Made by: KerfiveXᴺᴬ・☠#7273\a', end='', flush=True) # Update title of command prompt
...
╚═╝░░╚═╝╚══════╝╚═╝░░╚═╝╚═╝░░░░░╚═╝░░░╚═╝░░░╚══════╝╚═╝░░╚═╝""") # Print the title card
        ...
        self.slowType("Made by: KerfiveXᴺᴬ・☠#7273", .02) # Print who developed the code
        ...
        for i in range(num): # Loop over the amount of codes to check
            ...
            if os.name == "nt": # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleW(f"Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by: KerfiveXᴺᴬ・☠#7273") # Change the title
                print("")
            else: # If it is a unix system
                print(f'\33]0;Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by: KerfiveXᴺᴬ・☠#7273\a', end='', flush=True) # Change the title
        ...
        with open(self.fileName, "r", encoding="utf-8") as file: # Open the file containing the nitro codes
            for line in file.readlines(): # Loop over each line in the file
                ...
                if response.status_code == 200: # If the responce went through
                    ...
                    print(f" Invalido | {nitro} ") # Tell the user it tested a code and it was invalid
                    invalid += 1 # Increase the invalid counter by one

        return {"valid" : valid, "invalido" : invalid} # Return a report of the results
...
if __name__ == '__main__':
    ...
    Gen.main() # Run the main codenot a code found
